...
module u_rectangle(hole_size, cols, holes, w, mx=false, my=false, 
    mz=false, bx=0, bz=0, m=0, hm=0.25, verbose=false) {
    bx = border(bx, hole_size/2);
    bz = border(bz, hole_size/2);
    l = hole_size * (holes * 2) - hole_size + bx * 2;
    h = hole_size * (cols * 2) - hole_size + bz * 2;
    mgn_l = margin(mx, l, m);
    mgn_h = margin(mz, h , m);
    mgn_w = margin(my, w, m);
        ...
        for (a = [1:cols]) {
            for (h = [1:holes]) {
                translate([
                    ...
                    w+0.001,
                    ((a*hole_size + a*hole_size) - hole_size*1.5 + bz) - is_margin(mz, m)
                    ]) {
                    rotate([90,0,0]) {
